...
public class ComApiMock {
    private static Integer lib;
    private static ComApiMock[] comPorts = new ComApiMock[256];
    ...
    DeviceType[] pDevices = new DeviceType[9];
    private boolean[][] markers = new boolean[9][96];
    private byte pComPort;
    private int pBaudRate;

    private ComApiMock(int comPort, BaudRate baudRate) throws IllegalArgumentException {
        ...
        System.out.println("COM-object created. Port: " + String.valueOf(comPort));
    }
    ...
    private synchronized static void initializeDLL(int timeout) throws UnsatisfiedLinkError {
        if (lib == null) {
            ...
            System.out.println("DLL library initialized");
        }
    }

    private static void setTimeout(int timeout) throws IllegalArgumentException {
        lib = new Integer(timeout);
        System.out.println("Timeout for COM port set: " + String.valueOf(timeout));
    }

    @SuppressWarnings("unused")
    private static int getTimeout() throws IllegalArgumentException {

        System.out.println("Timeout reqest processed");
     ...
     * @throws IOException
     */
    public synchronized static ComApiMock connect(DeviceType deviceType, int comPort, BaudRate baudRate, int timeout,
        ...
        System.out.println("Try to register device on COM " + comPort + " , NetID " + netId);
        if (comPorts[comPort] != null) {
            if (comPorts[comPort].pDevices[netId] == null) {
                comPorts[comPort].pDevices[netId] = deviceType;
                System.out.println("New device appended to existed COM port. NetID: " + String.valueOf(netId)
                        + ". Count: " + comPorts[comPort].countDevices());
            ...
            ComApiMock.initializeDLL(timeout);

            ComApiMock comApi = new ComApiMock(comPort, baudRate);
            comApi.pDevices[netId] = deviceType;
            System.out.println("Try to add device: " + comApi.pDevices[netId]);
            System.out.println("New device appended to new COM port. NetID: " + String.valueOf(netId) + ". Count: "
                    + comApi.countDevices());
            System.out.println("Thead: " + Thread.currentThread().getId());
            int retval = 0; // lib.MC_Open_ComPort(comApi.pHandle, comApi.pComPort, comApi.pBaudRate);
            ...
            System.out.println("New COM port registered in system. Number: " + String.valueOf(comPort));
    ...
    public synchronized void disconnect(int netId, boolean lock) throws IOException, IllegalArgumentException {
        ...
        if (hasSingleDevice()) {
            ...
            System.out.println("COM-port closed. Number: " + String.valueOf(pComPort));
            comPorts[pComPort] = null;
            System.out.println("COM-port unregistered from system. Number: " + String.valueOf(pComPort));
        } else {
            pDevices[netId] = null;
        }
    }
    ...
    private void unlock(int netId, String password) throws IOException, IllegalArgumentException {
        ...
        int retval = 0;// lib.MC_Unlock_Device(pHandle.getValue(), (byte) netId, password, pError);

        if (retval != 0) {
            throw new IOException(libErrorCode(retval, pError.getValue(), pDevices[netId]));
        }

        System.out.println("Device unlocked: " + String.valueOf(netId));
    }
    ...
    private void lock(int netId) throws IOException, IllegalArgumentException {
        ...
        int retval = 0;// lib.MC_Lock_Device(pHandle.getValue(), (byte) netId, pError);

        if (retval != 0) {
            throw new IOException(libErrorCode(retval, pError.getValue(), pDevices[netId]));
        }

        System.out.println("Device locked: " + String.valueOf(netId));
    }
    ...
    public boolean[] getValuesArray(int netId, ElementType elementType) throws IllegalArgumentException, IOException {
        for (int i = 0; i < markers[netId].length; i++) {
            System.out.println(String.valueOf(i) + ": " + String.valueOf(markers[netId][i]));
        }
        return markers[netId];
    }

    public boolean getValue(int netId, ElementType elementType, int index)
            throws IllegalArgumentException, IOException {
        System.out.println("Get value of NetID: " + String.valueOf(netId) + ". Marker: " + String.valueOf(index)
                + ". Value: " + String.valueOf(markers[netId][index]));
    ...
    public void setValue(int netId, ElementType elementType, int index, boolean enabled)
        ...
        markers[netId][index] = enabled;
        System.out.println("Set value: " + String.valueOf(enabled));
        int retval = 0;// lib.MC_Write_Object_Value(pHandle.getValue(), (byte) netId, elementType.val(), (short) index,
                       // length, bData);
    ...
    private static String libErrorCode(int errorCode, int deviceErrorCode, DeviceType type) {
        switch (errorCode) {
            ...
            case 6:
                int sysError = 0;// lib.GetLastSystemError();
    ...
    private String countDevices() {
        int retval = 0;
        for (DeviceType d : pDevices) {
            System.out.println("Device instance: " + d);
            if (d != null) {
                retval++;
            }
        }
